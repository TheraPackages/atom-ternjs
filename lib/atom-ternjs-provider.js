...
class Provider {
        ...
        manager.client.completions(atom.project.relativizePath(editor.getURI())[1], {
          ...
          for (let obj of data.completions) {
            ...
            if (packageConfig.options.useSnippetsAndFunction && obj._hasParams) {
              console.log('packageConfig.options.useSnippetsAndFunction');
  ...
  forceCompletion() {
    ...
    this.force = true;
    atom.commands.dispatch(atom.views.getView(atom.workspace.getActiveTextEditor()), 'thera-autocomplete-plus-plus:activate');
    this.force = false;
  }
